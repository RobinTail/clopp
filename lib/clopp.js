...
exports.preprocess = function (grunt, src, options)
{
    ...
    {
        regex = regexps[scanFileTypes(contents, regex, grunt)] || regex;
    }
...
function scanFileTypes (source, regexs, grunt)
{
    var regexType = "array";
    var regexs_ = regexs;
    ...
    for (var i = 0; i < regexs.length; i++)
    {
        ...
        source = source.replace(regexps.all.filetype, function (match, type)
        {
            type_ = type.toLowerCase();
            grunt.verbose.write("Found " + "#filetype".cyan + " tag, setting the current filetype to " + type_.cyan + "\n");
            return "";
        });
    }
...
function scanReplaceDefinitions (source, regexs, grunt)
{
    ...
    for (var i = 0; i < regexs.length; i++)
    {
        ...
        for (var prop in definitions)
        {
            ...
            contents = contents.replace(new RegExp('(\\$\\()(' + prop + ')(\\))', 'gi'), function(match, pre, include, post) {
                amt++;
